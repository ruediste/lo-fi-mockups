...
/**
 * Converts a {@link WebJarsResourceReference} into a relative
 * {@link ExtendedURL} (with the Context Path added).
 *
 * @version $Id: adf0f62cd557b6a33bfca236c04130a4ed207109 $
 * @since 7.1M1
 */
@Component
@Singleton
public class WebjarsResourceReferenceSerializer
    ...
    public ExtendedURL serialize(WebJarsResourceReference resourceReference) {
        ...
        segments.add("lofimockups");

        // Add the namespace
        segments.add(resourceReference.wiki);

        // Add the resource path segments
        segments.add("" + resourceReference.pageNames.size());
        segments.addAll(resourceReference.pageNames);
        segments.add(resourceReference.attachment);
        segments.add(resourceReference.zipPath);

        // Add all optional parameters
        ExtendedURL extendedURL = new ExtendedURL(segments, resourceReference.getParameters());

        // Normalize the URL to add the Context Path since we want a full relative URL
        // to be returned.
